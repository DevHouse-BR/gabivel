...
include(ABSPATH . 'wp-content/themes/Gabivel/includes/widget_busca_carros.php');
function limpa_shortcode( $atts, $content = null ) {
    return $content;
}
add_shortcode('1of2', 'limpa_shortcode');
add_shortcode('1of2_end', 'limpa_shortcode');
//add_shortcode('clear', 'limpa_shortcode');
add_shortcode('divider', 'limpa_shortcode');
add_shortcode('layerslider', 'limpa_shortcode');

function limpa_clear($content){
    $content = do_shortcode($content);
    return str_replace("[clear]", "<br><br>", $content);
}
add_filter( 'the_content', 'limpa_clear' );