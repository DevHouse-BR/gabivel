...
function setHistoryLinks(targets){
    for(var i=0; i<targets.length; i++){
        $(targets[i]+' a[target!="_blank"]:not(.modal, .gallery a, a[href$=\'.jpg\'])').each(function(){
...
function init(){
    ...
    if(!historySupport || !historyApiEnable){
        ...
        setHistoryLinks(new Array('#menu-container', '#contentBox', '#contentBoxScroll', '#logo'));
...
function loadTweets(){
   var url = "http://twitter.com/status/user_timeline/"
         + twitterUser + ".json?count="+twitterCount+"&callback=?";
...
function ghostModeMouseMove(){
    ...
    ghostTimer = setTimeout(gotoGabivel, ghostModeTime);
}

var ghost = false;
var ghostTimer;
function gotoGabivel(){
    var addtionalQuery = '';
    if(useFullImage)
    ...
    if(ghostModeText)
        $('body').append($('<div class="ghostModeActive">Gabivel Mode Active</div>').css({opacity:'0'}));
...
function setContentHeight(){
    ...
    $('#content').css({top:conentTop, left:((winW-contentWidth)/2-20)+'px', height:(winH-$('#footer').height()-60-conentTop)+'px'});
...
function openPage(getURL, reload){
    ...
    if($('#contentBoxContainer').html()=='')
    {
        if(frontPage!='' && pageLoadingURL=='/'){
            ...
            $.ajax({
                'url' : pageLoadingURL + addionalCharacter(pageLoadingURL) + 'info=title',
                type : 'post',
                udata : pageLoadingURL,
                ureload : reload
            }).done(titleOnLoaded); 
