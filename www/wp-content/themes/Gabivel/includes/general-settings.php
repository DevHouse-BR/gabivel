...
<script type="text/javascript" src="<?php echo get_template_directory_uri(); ?>/includes/uploadify/jquery.uploadify-3.1.min.js"></script>
...
<script type="text/javascript">
	...
	function getImageManager(){
		...
		//$('#imageManager').show().css({opacity:'1'});
		...
		$("#settingsUploader").html("");
		$("#settingsUploader").append('<div id="settingsFileUpload">Upload doesnt work. You have a problem with your javascript</div>');
		$("#settingsFileUpload").uploadify({
			'swf': '<?php echo get_template_directory_uri(); ?>/includes/uploadify/uploadify.swf',
			'uploader': '<?php echo $pURL ?>/includes/uploadify/upload_settings_image.php',
			'multi': true,
			'width': 200,
			'height':30,
			'auto': true,
			...
			'onUploadSuccess':function(fileObj, data, response){
				...
				if(jdata.status=='OK')
				{
					$('#imageList tbody').prepend(jdata.html);
				}else{
					alert(jdata.ERR);
				}
			}
		...
		$('#imageManager').show().css({opacity:'0', left:'830px', top:'20px'}).stop().animate({opacity:'1'}, 300, function(){});
	}
	function themeCheck(){
	...
	function detailGallery(no, name){
		...
		$("#fileUploadGalleryDetail").uploadify({
			'swf': '<?php echo get_template_directory_uri(); ?>/includes/uploadify/uploadify.swf',
			'uploader': '<?php echo $pURL ?>/includes/uploadify/upload_background_image.php',
			'multi': true,
			'width': 120,
			'height': 30,
			'auto': true,
			'formData': {GALLERYID:no},
			'fileTypeDesc' : 'Image Files',
			'fileTypeExts': '*.jpg;*.jpeg;*.gif;*.png',
			'displayData': 'speed',
			'onUploadSuccess'  : function(fileObj, data, response) {
	...
	function setForm(data)
	{
		...
		//$("#rbcontent").find(":checkbox").iButton("repaint");
	...
	function thumUploader(me){
		...
		if($('#imageID'+imgID+' td:nth-child(2) .thumbUplodifyWrap').length==0)
		{
			...
			$("#thumbUplodify").uploadify({
				'swf': '<?php echo get_template_directory_uri(); ?>/includes/uploadify/uploadify.swf',
				'uploader': '<?php echo $pURL ?>/includes/uploadify/upload_background_thumb.php',
				'multi': false,
				'auto':true,
				'width':400,
				'height':30,
				'formData': {'IMAGEID':imgID},
				'fileTypeDesc' : 'Image Files',
				'fileTypeExts': '*.jpg;*.jpeg;*.gif;*.png',
				'onUploadSuccess'  : function(fileObj, data, response) {
					var jdata = $.parseJSON(data);
